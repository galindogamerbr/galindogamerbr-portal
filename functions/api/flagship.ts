...
export const onRequestGet: PagesFunction<Env> = async (context) =>
  withEdgeCache(context.request, (promise) => context.waitUntil(promise), async () => {
    const videos = await getRecentPlaylistVideos(context.env, FLAGSHIP_PLAYLIST_ID, 7)
    ...
    if (cached) {
      return json(
        { videos: [{ videoId: cached.video_id, title: cached.title, thumbnailUrl: cached.thumbnail_url }] },
